...
class Influence:
	...
	def update(self):
		...
		self.influence_cities = [[0 for x in range(self.height)] for x in range(self.width)]
		...
		for q in range(self.width):
			for r in range(self.height):
				...
				if cell is None:
					...
					influence_map = None
					if cell.visible:
						influence_map = self.influence_cities
					else:
					...
					piece_type = the_situation.get_piece_type(cell.piece_type_id)
					self.expand_influence(influence_map, location, \
	...
	def expand_influence(self, influence_map, location, intensity, step_length, nb_steps, terrains):
		...
		def neighbors(a):
			...
			for direction in algos.directions:
				qd, rd = direction
				b = qa + qd, ra + rd
				if the_situation.is_in_map(b):
					tile = the_situation.get_tile(b)
					if tile.terrain in terrains:
						result.append(b)
			return result
		def crossable(x):
			return True
		reachable, came_from, path_len = algos.breadth_first_search_all(location, depth, neighbors, crossable)
		reachable.append(location)
		path_len[location] = 0
		for (q, r) in reachable:
			influence_map[q][r] = influence_map[q][r] + \
					float(intensity) / (step_length * nb_steps) * float(step_length * nb_steps - path_len[(q,r)])

	def expand_influence_old(self, influence_map, location, intensity, step_length, nb_steps, terrains):
		q, r = location
		influence_map[q][r] = influence_map[q][r] + intensity
		for i in range(nb_steps):
			intensity = intensity / 2.0
			for x in range(step_length):
				radius = i * step_length + x + 1
				for (qa, ra) in algos.cube_ring(location, radius):
					if self.the_situation.is_in_map((qa, ra)):
						terrain = self.the_situation.get_terrain((qa, ra))
						if terrain in terrains:
							influence_map[qa][ra] = influence_map[qa][ra] + intensity
	...
	# TODO: free_cities!
	def get_influence_cities(self, location):
		q, r = location
		return self.influence_cities[q][r]
	...
	def show(self):
		...
		print "INFLUENCE CITIES:"
		self.show_influence_map(self.influence_cities)
