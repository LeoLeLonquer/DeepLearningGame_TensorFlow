# Using Deep Q Networks to Learn Empire Strategies
#### Valentin Chatelard - François Manach - Aurélien Lainé - Nabil Sellam
...
The network was trained on matrix observed from the game at each time step. We preprocessed the matrix from the server to be converted in numbers. 
The architecture of the network is described below. The first layer concolves the input image with an 4x4x4x16 kernel at a stride size of 2. The output is then put through a 2x2 max pooling layer. The second layer convolves with a 3x3x16x16 kernel at strude of 1. We then max pool again. The last hidden layer consists of 144 fully connected ReLU nodes.
![model](http://s32.postimg.org/69g1x5qo5/modele.png)

The output layer, obtained with a simple matrix multiplication, has the same dimensionality as the number of valid actions which can be performed in the game, where the 6th index always corresponds to doing nothing. The values at this output layers corresponds to the highest Q value using a ϵ greedy policy.
...
We start training by choosing actions uniformly at random for 400 time steps, without updating the network weights. This allows us to populate the replay memory before training begins. After that, we linearly anneal ϵ from 1 to 0.05 over the course of the next 5000,000 frames. During this time, at each time step, the network samples minibatches of size 100 from the replay memory to train on, and performs a gradient step on the loss function described above using the Adam optimization algorithm with a learning rate of 0.000001. After annealing finishes, the network continues to train indefinitely, with ϵ fixed at 0.05.

A DigitalOcean instance of 8Go of Ram and 4 CPUs was used to efficiently conduct training on a CPU. We implemented the DQN in Google's newly released TensorFlow library.

## 6. Results
We found that for Empire, results were achived after 11.000 turns which corresponds to about 3 days of game. Qualitatively, the network played at a level of a basic player, usually beating a basic AI but beaten by a more complex AI (using decision graphs). As can be seen, the maxium Q value inscreases over time. This indicates improvement as it means that the network is expecting to receive a greater reward per game as it trains for longer. In theory, as we continue to train the network, the value of the maximum Q value should plateau as the network reaches an optimal state.

### Using a non optimized oracle
![bad oracle](http://s32.postimg.org/b20zmlww5/oracle_sans.png)

### Using better oracle
![good oracle](http://s32.postimg.org/lhpzxv4jp/oracle_avec.png)

## 7. Conclusions
We utilized deep Q learning to train a neural network to play Empire. Qualitatively, the trained network appeared to perform a basic level. We are still working on training the system to play better, and will hopefully report better results in the near future.

Significant improvements on the current network architecture are possible. For example, our implementation of max pooling may have been unnecessary, as it caused the convolution kernels in the deeper layers to have a size comparable to that of the previous layer itself. Hence, max pooling may have discarded useful information. Given more time, it would be nice to perform more tuning over the various network parameters, such as the learning rate, the batch size, the replay memory, and the lengths of the observation and exploration phases. Additionally, we could check the performance of the network if the game parameters are tweaked (city density etc.). This would yield important information about the network’s ability to extrapolate to new situations.
